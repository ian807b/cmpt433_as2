...
void tradeArrays(double *array1, double *array2, int size)
{
	...
	// original for loop:
	// for (i = size-1; i >= 0; i--)
	// However, if i = 0, i-- results in i = INT_MAX (maximum unisigned integer value) due to wraparound
	// array1/2 + INT_MAX results in a segfault due to accessing memory past the array boundary
	for (i = size-1; i > 0; i--) { 
